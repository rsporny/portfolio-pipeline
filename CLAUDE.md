...
- Commits: conventional commits, in English. The weekly CI opens one PR against the site repo bundling the rendered devlog with its `raw/` + `memory/` state; it commits nothing back to this repo.
...
- `uv run pipeline provenance sign --week <YYYY-Wnn>` — v0.5: GPG-sign a published entry (run on the PR branch before merge), write the `.sig` sidecar + `provenance:` front matter via the adapter, and record the merkle leaf. Also `--slug` for custom entries
- `uv run pipeline provenance anchor` — anchor the current merkle root via `provenance.anchor.backend` (`null`/`file`/`cardano` testnet)
- `uv run pipeline provenance verify [--chain]` — recompute leaves + signatures + root (offline); `--chain` also reads anchors back. Non-zero on any failure
- `uv run pipeline provenance show [--slug <slug>]` — print leaf hashes, signatures, inclusion proofs, and anchors
- `uv run pytest` — tests
