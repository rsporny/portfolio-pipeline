...
For a repo with an active thread the collector adds **selective deep context** — each PR's review discussion and linked issues (`activity.json` is `schema_version: 3`). That input is anonymized in the collector — third-party logins/names/`@mentions`/`Co-authored-by` are masked to `[collaborator]` before `raw/` is written — so it informs understanding only and is never quoted or attributed.
