...
warn, not a hard gate (framing is a judgement call).

**v0.6.2** — per-repo indexer scoping (memory-correctness fix). The indexer runs
once per repo, but Stage A emits a single cross-repo initiative list and every
repo's indexer received *all* of it — so an unrelated repo's indexer would create
a thread for another repo's work in its own registry. Live symptom: one thread id
(`reader-output-and-publishing-automation`) written into three registries, each
with a different title, which then surfaced the same id several times in one week's
focus candidates. `transform_week` now scopes each repo's indexer to only the
